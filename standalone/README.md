...
const config = createDefaultConfig({
  ...
  csv: { delimiter: ",", decimal: ".", thousands: "" },
});
```
...
const url = buildReportUrl(config); // ?report=<base64>
const incoming = readReportFromUrl(); // PivotConfig | null on page load
...
const matrix = matrixFromResult(result, { header: "Q3 revenue", footer: "Acme" });
exportMatrix(matrix, "csv", "q3-revenue"); // "csv" | "tsv" | "html" | "json" | "excel" | "pdf"
```
...
const engine = createBackendEngine({
  baseUrl: "https://api.example.com/pivot",
...
const hybrid = createHybridEngine({ baseUrl: "/api/pivot", threshold: 100_000 });

// Any backend at all — you write two functions:
const custom = createCustomEngine({
  async query(q) {
    return await myGraphQlPivot(q); // must return a PivotResult
  },
  async drillThrough(q) {
    return { rows: await myRecords(q), total: 1234 };
  },
});

// 1GB+ files registered server-side and streamed:
const big = createServerAggregationEngine({ baseUrl: "/api/pivot", datasetId: "orders-2024" });
```
...
const api = createMockPivotApi({ datasets: { sales: { rows: sampleData, fields: sampleFields } } });
globalThis.fetch = api.fetch; // same routes and payloads as the real service

const engine = createBackendEngine({ baseUrl: "/api/pivot" });
...
expect(result.rowHeaders.map((h) => h.caption)).toContain("EMEA");
```
